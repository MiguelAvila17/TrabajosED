...
std::vector<std::string> ArbolFS::trashList() const {
    std::vector<std::string> r; for (auto& n : papelera_) r.push_back(n->nombre()); return r;
}
bool ArbolFS::restore(const std::string& nombre) {
    for (auto it = papelera_.begin(); it != papelera_.end(); ++it) {
        if ((*it)->nombre() == nombre) {
            if (!actual_->agregarHijo(*it)) return false;
            papelera_.erase(it);
            return true;
        }
    }
