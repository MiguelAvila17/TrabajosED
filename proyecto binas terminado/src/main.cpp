...
int main(){
    const char* kAppName = "proyecto binas terminado";
    const char* kAppVersion = "1.0.0";
    std::cout << kAppName << " v" << kAppVersion << " (" << __DATE__ << " " << __TIME__ << ")\n";
    ArbolFS fs;
    {
        std::ifstream f("filesystem.json");
        if (f) fs.load("filesystem.json");
    }
    std::string line;
    while (true) {
        std::cout << fs.pwd() << " > ";
        ...
        if (cmd=="exit"||cmd=="quit") break;
            ...
            std::cout << "proyecto binas terminado - Sistema de Archivos (CLI)\n";
            std::cout << "Uso:\n";
            ...
            std::cout << "  trash                     Lista elementos en papelera\n";
            std::cout << "  restore <nombre>          Restaura elemento desde papelera\n";
            std::cout << "  emptytrash                Vacía la papelera\n";
        ...
        else if (cmd=="trash") { for (auto& n : fs.trashList()) std::cout << n << "\n"; }
        else if (cmd=="restore") { std::string nombre; iss >> nombre; std::cout << (fs.restore(nombre)?"ok":"err") << "\n"; }
        else if (cmd=="emptytrash") { std::cout << (fs.emptyTrash()?"ok":"err") << "\n"; }
