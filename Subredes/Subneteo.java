...
public class Subneteo extends JFrame {
    ...
    private void calcular() {
        ...
        try {
            ...
            // Prácticas: Se restan 2 (Subred Zero y la última) según estándar clásico
            ...
            // Prácticos: Se resta ID de Red y Broadcast
            ...
            sb.append(String.format("%-5s %-18s %-35s %-18s\n", 
                      "No.", "ID de Red", "Rango de Hosts Útiles", "Broadcast"));

            // Limitar a 512 para evitar que la interfaz se bloquee al imprimir
            long limite = Math.min(subredesTeoricas, 512); 
            for (int i = 0; i < limite; i++) {
                ...
                sb.append(String.format("%-5d %-18s %-35s %-18s\n", 
                    (i + 1), 
                    longToIp(networkID), 
            ...
            if(subredesTeoricas > 512) {
                sb.append("\n... (Lista truncada a 512 subredes por rendimiento) ...");
            }

            areaResultado.setText(sb.toString());
            areaResultado.setCaretPosition(0); // Volver al inicio del texto
    ...
    // MÉTODOS DE SOPORTE PARA ARITMÉTICA DE BITS
    ...
    private String cidrToMask(int cidr) {
        // Generar una máscara de 32 bits basada en el prefijo CIDR
    ...
    public static void main(String[] args) {
        // Asegurar que la UI se ejecute en el hilo correcto
        SwingUtilities.invokeLater(Subneteo::new);
    }
}