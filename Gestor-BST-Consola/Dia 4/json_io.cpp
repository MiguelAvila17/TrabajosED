...
std::shared_ptr<Nodo> cargarJson(const std::string& contenido) {
    // Parser minimal para el esquema generado arriba (no general-purpose)
    // Solo soporta campos en el mismo orden.
    size_t i = contenido.find("\"id\":");
    if (i==std::string::npos) return nullptr;
    i += 5; size_t j = contenido.find(',', i); auto id = std::stoull(contenido.substr(i, j-i));
    i = contenido.find("\"nombre\":\"", j); if (i==std::string::npos) return nullptr; i += 11; j = contenido.find('"', i); auto nombre = contenido.substr(i, j-i);
    i = contenido.find("\"tipo\":\"", j); i += 9; j = contenido.find('"', i); auto tipoStr = contenido.substr(i, j-i);
    auto tipo = tipoStr=="carpeta"?TipoNodo::Carpeta:TipoNodo::Archivo;
    ...
    // Buscar hijos con un contador de llaves.
