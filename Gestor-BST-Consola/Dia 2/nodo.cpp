...
std::shared_ptr<Nodo> Nodo::padre() const { return padre_.lock(); }
void Nodo::setPadre(const std::shared_ptr<Nodo>& padre) { padre_ = padre; }

const std::vector<std::shared_ptr<Nodo>>& Nodo::hijos() const { return hijos_; }

bool Nodo::agregarHijo(const std::shared_ptr<Nodo>& hijo) {
    // Evitar nombres duplicados dentro de la misma carpeta
    for (const auto& h : hijos_) {
        if (h->nombre() == hijo->nombre()) return false;
    }
...
bool Nodo::eliminarHijoPorNombre(const std::string& nombre) {
    for (auto it = hijos_.begin(); it != hijos_.end(); ++it) {
        if ((*it)->nombre() == nombre) {
            hijos_.erase(it);
            return true;
        }
